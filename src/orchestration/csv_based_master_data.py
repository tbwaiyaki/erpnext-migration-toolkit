...
class CSVBasedMasterDataCreator:
    ...
    def create_uom(self, uom_name: str) -> bool:
        """
        Create UOM in ERPNext.
    ...
    def get_unique_items_from_csv(self) -> list[dict]:
        ...
        for _, row in unique_items.iterrows():
            ...
            # Capitalize UOM properly
            if pd.notna(row['unit']):
                unit = row['unit'].strip().capitalize()
            else:
                unit = 'Nos'
    ...
    def create_all_from_csv(self) -> dict:
        ...
        # Extract unique UOMs and create them first
        print("\n2. Creating required UOMs...")
        unique_uoms = set(item['uom'] for item in items_to_create)
        uoms_created = 0
        for uom in sorted(unique_uoms):
            if self.create_uom(uom):
                uoms_created += 1
                print(f"   ✓ {uom}")
        ...
        print("="*70)
        print(f"UOMs:      {uoms_created}/{len(unique_uoms)}")
        ...
        return {
            'uoms': {
                'total': len(unique_uoms),
                'created': uoms_created
            },
            'items': {
